...
class MultiHopSolver:
    def __init__(self, model_id="meta-llama/Llama-3.3-70B-Instruct", ex_num=2, subject="high_school_chemistry", generate_ex=True, review_doc=True, review_ex=True, zeroshot=False, summarize=False, PD_baseline=False, select_sol=False, zeroshot_RAG=False, extract=False):
        ...
        self.wandb_project = os.getenv('WANDB_PROJECT', f'final-{subject}')  # Default to project name if not set
        ...
        base_dir = os.path.join(os.path.dirname(__file__), f"results/final/{self.subject}")
        ...
        if model_id == "meta-llama/Llama-3.3-70B-Instruct":
            config_str = "70B_" + config_str
  
        self.output_dir = os.path.join(base_dir, config_str, self.timestamp)
...
Respond with only "RELEVANT" or "NOT_RELEVANT"."""
    ...
    def solve_subquestion_base(self, problem, subquestions, subsolutions, step_num):
    ...
    def solve_problem(self, subject, prob_num):
        ...
        try:
            ...
            if self.zeroshot:
                ...
                start = time.time()
                file_print("Solving with zero-shot approach...")
                documents = self.get_wiki_search_zeroshot(problem['question'], num_results=3)
                ...
                for step_num in range(1, len(subquestions) + 1):
                    ...
                    start = time.time()
                    solution = self.solve_subquestion_base(problem, subquestions, subsolutions, step_num)
                ...
                for step_num in range(1, len(subquestions) + 1):
                    ...
                    if self.select_sol:
                        ...
                        solution1 = self.solve_subquestion_base(problem, subquestions, subsolutions, step_num)
                        file_print(f"\nBase Solution for subquestion {step_num}:")
