...
class MultiHopSolver:
    def __init__(self, model_id="meta-llama/Llama-3.3-70B-Instruct", ex_num=2, generate_ex=True, review_doc=True, review_ex=True, zeroshot=False,  PD_baseline=False, select_sol=False, zeroshot_RAG=False, extract=False,trigger=False,trigger_value=0.8,filter=False,doc_num=10, wandb_run=None, PD_trigger=False, self_rag=False):
        ...
        self.wandb_entity = os.getenv('WANDB_API_KEY', 'minhae')  # Default to 'minhae' if not set
        ...
        if self.zeroshot:
            ...
            config_str = "query1_explanation_"+config_str
    ...
    def subquestion_trigger(self,problem):
        ...
        options = options[:len(problem['options'])]
        formatted_choices = [f"{i}: {choice}" for i, choice in zip(options, problem['options'])]
...
Summary:"""
    ...
    def get_wiki_search_results(self,problem, subquestion, query, num_results=3, review=False,subject=None):
...
"""
    ...
    def solve_problem(self, subject, prob_num, problem=None):
        base_dir = os.path.join(os.path.dirname(__file__), f"MMLU-Pro/{subject}")
        ...
        try:
            ...
            for print_func in [file_print, summary_print]:
                ...
                print_func("Choices: {}".format(problem['options']))
                options = ['A', 'B', 'C', 'D','E','F','G','H','I','J']
            ...
            if self.zeroshot:
                ...
                start = time.time()
                file_print("Solving with zero-shot approach...")
                documents = self.get_wiki_search_zeroshot(problem['question'], num_results=self.doc_num)
                ...
                if len(subquestions) == 0:
                    ...
                    for step_num in range(1, len(subquestions) + 1):
                        ...
                        start = time.time()
                        explanation,solution = self.solve_subquestion_carefully(problem, subquestions, subsolutions, search_queries, step_num, subject)
                        step_timing['solution_generation'] = time.time() - start
                        file_print(f"Explanation: {explanation}")
                ...
                if self.PD_trigger and 'direct' in PD_trigger.lower():
                    ...
                    if len(subquestions) == 0:
                        ...
                        for step_num in range(1, len(subquestions) + 1):
                            ...
                            start = time.time()
                            documents, select_num, distances, response = self.get_wiki_search_results(problem, subquestions[step_num - 1], search_queries[step_num - 1], num_results=self.doc_num, review=self.review_doc,subject=subject)
                            #documents, select_num = self.get_wiki_search_results(problem, subquestions[step_num - 1], initial_solutions[step_num - 1], num_results=3, review=self.review_doc)
                            ...
                            if self.filter:
                                ...
                                if self.select_sol:
                                    ...
                                    start = time.time()
                                    file_print("Solving subquestion without additional information...")
                                    solution1 = self.solve_subquestion_base(problem, subquestions, subsolutions, step_num, subject)
                                    file_print(f"\nSolve subquestion {step_num} with zeroshot:")
                                    file_print(solution1)
                                    solutions.append(solution1)
                                    step_timing['solution_generation_base'] = time.time() - start
                                    file_print("Running select_solution...")
                                    file_print(f"\nSolutions length: {len(solutions)}")
                                    solution_num = self.select_solution(problem, subquestions, subsolutions, step_num, solutions)
                                    file_print(f"Selected solution for subquestion {step_num}: {solution_num}")
                                    if solution_num == 2:
