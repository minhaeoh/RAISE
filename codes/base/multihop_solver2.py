...
class MultiHopSolver:
    def __init__(self, model_id="meta-llama/Llama-3.3-70B-Instruct", ex_num=2, generate_ex=True, review_doc=True, review_ex=True, zeroshot=False,  PD_baseline=False, select_sol=False, zeroshot_RAG=False, extract=False,trigger=False,trigger_value=0.8,filter=False,doc_num=10, wandb_run=None, PD_trigger=False):
        ...
        self.wandb_project = os.getenv('WANDB_PROJECT', 'MMLU-Pro')  # Default to project name if not set
        ...
        if wandb_run is not None:
            ...
            self.run = wandb.init(
                ...
                name=f"letter_query_explanation_{config_str}_{self.timestamp}",
                config={
                    "model_id": model_id,
                    "ex_num": ex_num,
    ...
    def get_wiki_search_results(self,problem, subquestion, query, num_results=3, review=False):
        ...
        for line in response.split("\n"):
            if "End of explanation".lower() in line.lower():
                response = line.lower().split("end of explanation")[0]
                break
            else:
                lines.append(line)
        response = "\n".join(lines)
        ...
        try:
            ...
            with torch.no_grad():
                query_embedding = self.p_encoder(**inputs).pooler_output.cpu().numpy()
...
"""
    ...
    def solve_problem(self, subject, prob_num, problem=None):
        base_dir = os.path.join(os.path.dirname(__file__), f"results4/{subject}")
        self.output_dir = os.path.join(base_dir, f"letter_query_explanation_{self.config_str}",self.timestamp)
