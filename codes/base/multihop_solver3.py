...
class MultiHopSolver:
    def __init__(self, model_id="meta-llama/Llama-3.3-70B-Instruct", ex_num=2, generate_ex=True, review_doc=True, review_ex=True, zeroshot=False,  PD_baseline=False, select_sol=False, zeroshot_RAG=False, extract=False,trigger=False,trigger_value=0.8,filter=False,doc_num=10, wandb_run=None, PD_trigger=False):
        ...
        self.wandb_project = os.getenv('WANDB_PROJECT', 'Final-MMLU-STEM')  # Default to project name if not set
        ...
        if self.zeroshot:
            ...
            if not self.generate_ex:
                if self.review_doc:
                    config_str = "only_R_Doc"
                else: 
                    config_str = "only_Doc"
            else:
                if self.review_doc and self.review_ex:
                    config_str = "R_Doc_Ex"
                elif self.review_doc:
                    config_str = "R_Doc"
                elif self.review_ex:
                    config_str = "R_Ex"
                else:
            ...
            if self.filter:
                config_str += "_filter"
            if self.PD_trigger:
                config_str = "PD_trigger_"+config_str
        #config_str = "prompt1_"+config_str    
        ...
        # Use provided wandb run or initialize new one
        if wandb_run is not None:
            ...
            self.run = wandb.init(
                ...
                name=f"solve_sbs_{config_str}_{self.timestamp}",
                config={
                    "model_id": model_id,
                    "ex_num": ex_num,
                    ...
                    "zeroshot_RAG": zeroshot_RAG,
                    "extract": extract,
                    "trigger": trigger,
                    "trigger_value": trigger_value,
                    "filter": filter,
                    "doc_num": doc_num,
                    "PD_trigger": PD_trigger
                }
            )
        else:
            print("Warning: WANDB_API_KEY not found in .env file. Skipping wandb initialization.")
            self.run = None
    ...
    def subquestion_trigger(self,problem):
        ...
        choices_text = ", ".join(formatted_choices)
    ...
    def select_solution(self, problem, subquestions, subsolutions, step_num, solutions):
        ...
        base_prompt = f"""You are solving a complex problem step by step. You will be given:
            ...
            4. 2 solutions for the current subquestion

            Your task:
            - Carefully read the original question, any previous solutions, and the current subquestion
            - Select the better solution for the current subquestion
            - Answer only one of the following: Solution 1, Solution 2
        ...
        base_prompt += f"""
                ...
                """
        print("complete adding 2 solution selections")
        prompt = base_prompt + "Better solution is"

        response = self.generate_text(prompt,base_prompt, max_length=8)
        print(f"response: {response}")
        correct_sols = re.findall(r'\d+', response)
        if len(correct_sols) == 0:
            correct_sol = 2
        else:
            correct_sol = int(correct_sols[0])-1
        print(f"correct solution: {correct_sol}")
        return correct_sol
...
# """
    ...
    def generate_next_subquestion(self, problem, subquestion, subsolution, step_num, subject):
        options = ['A', 'B', 'C', 'D']
        formatted_choices = [f"{i}. {choice}" for i, choice in zip(options, problem['choices'])]
        choices_text = ", ".join(formatted_choices)

        base_prompt = f"""
        You will be given a multiple choice question about subject {subject}.
        Given the original question and (if available) the solutions to previously answered subquestions, your task is to generate the next essential subquestion that should be solved to help answer the original question. Then, generate a relevant search query that would help answer this subquestion.

If the original question can already be answered — either directly or by reasoning with the current subsolutions — then no further subquestions are needed.
...
DONE
        ...
        if step_num>1:
            for i in range(step_num-1):
                base_prompt += f"""\n
                Subquestion {i+1}: {subquestion[i]}
                Solution for Subquestion {i+1}: {subsolution[i]}0
                """
        base_prompt += f"""\n
        Now generate your ouput in one of the following two formats:
        Option A — if a new subquestion is needed:
        Output: Option A
        Subquestion N: [write the subquestion here]  
        Search Query for Subquestion N: [write the search query here]

        Option B — if no more subquestions are needed and final reasoning is now possible
        Output: Option B
        DONE

        You must choose Option B if:
- The final answer can be computed now using existing subsolutions, OR
- No further reasoning steps are required to connect current information to the final answer.
        """
        prompt = base_prompt + "\n\nOutput:"
        # Get raw response and clean it
        raw_response = self.generate_text(prompt,base_prompt, max_length=100)
        ...
        index = 0
        for i, line in enumerate(lines):
            if 'option b' in line.strip().lower():
                done = True
                break
            elif 'option a' in line.strip().lower():
                index = i
                break
        ...
        if done:
            ...
            for i, line in enumerate(lines[index:]):
                if line.strip().startswith("Subquestion"):
                    next_subquestion = line.split(":")[1].strip()
                elif line.strip().startswith("Search Query for Subquestion"):
                    next_search_query = line.split(":")[1].strip()
                    break
               
        return raw_response, next_subquestion, next_search_query, done
...
Summary:"""
    ...
    def get_wiki_search_results(self,problem, subquestion, query, num_results=3, review=False):
        ...
        try:
            ...
            search_num = num_results * 3 if review else num_results
            ...
            for i, (score, idx) in enumerate(zip(distances[0], indices[0])):
                ...
                if review:
                    ...
                    if self.review_document(query, content):
                        if self.extract:
                            doc = self.extract_knowledges(problem,subquestion,content)
    ...
    def get_wiki_search_zeroshot(self, problem, subquestion,query, num_results=3):
        """Get search results using DPR and FAISS"""
        try:
            print(f"\nDebug: Starting search for query: '{query}'")
            ...
            for i, (score, idx) in enumerate(zip(distances[0], indices[0])):
                ...
                doc = f"Document {i+1} (score: {score:.2f}) [Title: {title}]:\n{content}"
                if self.extract:
                    doc = self.extract_knowledges(problem,subquestion,doc)
                documents.append(doc)
...
Respond with only "RELEVANT" or "NOT_RELEVANT"."""
    ...
    def extract_knowledges(self,problem,subquestion,document):
        """Extract knowledges from the documents for the current subquestion"""
        options = ['A', 'B', 'C', 'D']
        formatted_choices = [f"{i}. {choice}" for i, choice in zip(options, problem['choices'])]
        choices_text = ", ".join(formatted_choices)

        base_prompt = f"""Your job is to summarize knowledges from the given document that are helpful to solve the subquestion. You will be given the original question, a subquestion, and a knowledge document.

        Your task is:
        - Only extract and summarize knowledges from the given document that are relevant to the subquestion.
        - Include every important knowledge from the document.
        - Do not solve the original question.
        - If there is no relevant knowledge in the document, just return "No relevant knowledges found."
        - End your output with "END"

        Format:
        Extracted Knowledge: [Write the extracted knowledges here]
        END
        
        Here is the input:
        Original Question: {problem['question']} | Options: {choices_text}
        Subquestion: {subquestion}
        Knowledge Document: {document}"""
        prompt = base_prompt + "\n\nExtracted Knowledge:"

        response = self.generate_text(prompt,base_prompt,max_length=200)
        
        # Parse the generated examples
        raw_document = re.split(r"```+", response)[0]
        raw_document = re.split(r'"""+', raw_document)[0]
        knowledges_list = []
        current_knowledge = []

        print("Raw Document: ", raw_document)
        for line in raw_document.split('\n'):
            line = line.strip()
            if not line:
                continue
            if line.startswith("Extracted Knowledge"):
                if len(current_knowledge) >= 1:
                    knowledge = "\n".join(current_knowledge)
                    knowledge = knowledge.split(":")[1].strip()
                    knowledges_list.append(knowledge)
                    current_knowledge = []
                if "END" in line:
                    line = line.split("END")[0]
                    current_knowledge.append(line)
                    knowledge = "\n".join(current_knowledge)
                    knowledge = knowledge.split(":")[1].strip()
                    knowledges_list.append(knowledge)
                    current_knowledge = []
                ...
                if ":" in line:
                    ...
                    current_knowledge.append(line.strip())
            elif "No relevant knowledges found." in line and len(knowledges_list) == 0:
                return ""

        return "\n".join(knowledges_list)
    ...
    def generate_examples(self, problem, documents, subquestions, step_num):
        ...
        response = self.generate_text(prompt,base_prompt,max_length=400)
    ...
    def solve_subquestion(self, problem, subquestions, examples, subsolutions, step_num, subject):
        ...
        base_prompt = f"""You are solving a multiple-choice question about {subject}. Question is decomposed into several subquestions. You will be given:
            ...
            [Solution for the current subquestion]
            End of Step {step_num} solution.
    ...
    def solve_subquestion_carefully(self, problem, subquestions, subsolutions, search_queries, step_num, subject):
        """Steps 3-5: Solve each subquestion step by step with zeroshot COT"""
        options = ['A', 'B', 'C', 'D']
        formatted_choices = [f"{i}. {choice}" for i, choice in zip(options, problem['choices'])]
        choices_text = ", ".join(formatted_choices)

        base_prompt = f"""You are solving a multiple-choice question about {subject}. Question is decomposed into several subquestions. Your task is to solve the current subquestion. 
        ...
        You will be given:
            ...
            Your task:
            - Carefully read the original question, any previous subquestion and their solutions, and the current subquestion
            - First generate the explanation for the search query.
            - Then, use the explanation to solve the current subquestion.
            - End your output with "End of Step {step_num} solution."
            - Only solve the current subquestion.

            Output format:
            Step {step_num} Explanation:
            [Explanation for the search query]
        ...
        index2 = 0
        for i, line in enumerate(responses):
            if f'step {step_num} solution' in line.lower():
                index1 = min(i, len(responses))
                break
        for i, line in enumerate(responses):
            if f'end of step {step_num} solution' in line.lower():
                index2 = min(i+1, len(responses))
                break
        ...
        if index1 != 0 and index2 != 0:
            explanation = "\n".join(responses[:index1])
            solution = "\n".join(responses[index1:index2])
        elif index1 != 0 and index2 == 0:
            explanation = "\n".join(responses[:index1])
            solution = "\n".join(responses[index1:])
        elif index1 == 0 and index2 != 0:
            explanation = "Can't find the explanation"
            solution = "\n".join(responses[index2:])
        else:
            explanation = "Can't find the explanation and solution"
            solution = "\n".join(responses)
...
"""
    ...
    def solve_problem(self, subject, prob_num, problem=None):
        base_dir = os.path.join(os.path.dirname(__file__), f"results3/{subject}")
        self.output_dir = os.path.join(base_dir, f"solve_sbs_{self.config_str}",self.timestamp)
        if prob_num == 1:
        ...
        try:
            ...
            if prob_num == 1:
                ...
                summary_print(f"Extract: {self.extract}")
            ...
            for print_func in [file_print, summary_print]:
                ...
                formatted_choices = [f"{i}. {choice}" for i, choice in zip(options, problem['choices'])]
                choices_text = ", ".join(formatted_choices)
                print_func("Choices: {}".format(choices_text))
                correct_answer = options[problem['answer']]
            ...
            if self.PD_trigger and 'direct' in PD_trigger.lower():
                ...
                final_answer = self.solve_with_zeroshot(problem)
                ...
                while not done:
                    step_timing = {}
                    file_print(f"Step {step_num}: Generating subquestions and search queries...")
                    start = time.time()
                    raw_response, next_subquestion, next_search_query, done = self.generate_next_subquestion(problem, subquestions, subsolutions, step_num, subject)    
                    step_timing['generate_next_subquestion'] = time.time() - start
                    file_print(f"raw_response: \n{raw_response}\n")
                    if not done :   
                        file_print(f"next_subquestion: {next_subquestion}")
                        file_print(f"next_search_query: {next_search_query}")
                        subquestions.append(next_subquestion)
                        search_queries.append(next_search_query)
                        file_print(f"Retrieve documents for subquestion {step_num}...")
                        start = time.time()
                        documents, select_num, distances, response = self.get_wiki_search_results(problem, subquestions[step_num - 1], search_queries[step_num - 1], num_results=self.doc_num, review=self.review_doc)
                        step_timing['retrieve_documents'] = time.time() - start 
                        file_print(f"Explanation generated: {response}")
                        if self.filter and select_num == 0:
                            ...
                            if self.generate_ex:
                                ...
                                file_print(f"\nOnly using documents for Subquestion {step_num}.")
                                start=time.time()
                                solution = self.solve_subquestion_with_docs(problem, subquestions, 
                                                                            documents, subsolutions, step_num, subject)
                                step_timing['solution_generation'] = time.time() - start
                        ...
                        file_print("No more subquestions needed.")
            ...
            for i, line in enumerate(final_answer_lines):
                if 'the final answer is' in line.lower():
                    try:    
                        ...
                        answer_part = line.lower().split('the final answer is')[1].strip()
                        # Extract the first letter (A-J)
                        letters = re.findall(r'[A-J]', answer_part)
                        if letters:
                    ...
                    try:
                        ...
                        letters = re.findall(r'[A-D]', answer_line.split('.', 1)[0])
                        if letters:
                            predicted_answer = letters[0]
                            break
                    except:
