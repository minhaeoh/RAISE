...
class MultiHopSolver:
    ...
    def subquestion_trigger(self,problem):
        ...
        formatted_choices = [f"{i}: {choice}" for i, choice in zip(options, problem['choices']['text'])]
...
# """
    ...
    def get_wiki_search_results(self,problem, subquestion, query, num_results=3, subject=None):
        ...
        try:            
            ...
            inputs = self.q_tokenizer(subquestion, return_tensors="pt", padding=True, truncation=True)
...
Respond with only "RELEVANT" or "NOT_RELEVANT"."""
    ...
    def solve_subquestion(self, problem, subquestions, examples, subsolutions, step_num, subject):
        ...
        #     Question: {problem['question']} | Options: {choices_text}"""
        base_prompt = f"""You are solving a multiple-choice question. Question is decomposed into several subquestions. You will be given:
    ...
    def solve_with_zeroshot(self, problem, subject):
        ...
        base_prompt = f"""You are solving a multiple choice question.
Question: {problem['question']} | Options: {choices_text}
...
"""
    ...
    def solve_problem(self, subject, prob_num, problem=None):
        base_dir = f"/home/minhae/multihop-RAG2/results/ARC/{subject}"
        ...
        try:
            ...
            if self.zeroshot:
                file_print(f"Question ID: {problem['id']}")
                start = time.time()
            ...
            summary_print(f"{prob_num:^10} | {'Error':^10} | {'-':^10} | {problem['answerKey']:^10} | {'-':^11}")
            raise
