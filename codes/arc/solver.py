...
class MultiHopSolver:
    ...
    def solve_with_zeroshot_COT(self, problem):
        ...
        base_prompt = f"""You are solving a multiple choice question. Think step by step and then finish your answer with "The final answer is (X)" where X is the correct letter choice.
    ...
    def generate_subquestions(self, problem):
        ...
        prompt = base_prompt + "\n\nSubquestion 1:"
        try:
            response = self.generate_text(prompt, base_prompt, max_length=600)
            subquestions = []
            queries = []
            subquestion = ""
            query = ""
            ...
            query_cur = False
            for line in response.split("\n"):
                if not subq_cur and not query_cur and line.startswith("Subquestion"):
                    subq_cur = True
                    subquestion = line.split(":")[1].strip()
                elif subq_cur and not query_cur and line.startswith("Search Query for Subquestion"):
                    query_cur = True
