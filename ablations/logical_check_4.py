...
def evaluate_with_gpt4(data):
    ...
    print("Got client")
    prompt = f"""You are given the following three items:
...
if __name__ == "__main__":
    ...
    json_path = f"/home/minhae/multihop-RAG2/ablation/gpqa/llama/{args.mode}.json"  # Replace with your actual JSON file path
    ...
    for data in tqdm(full_data):
        ...
        if "No relevance at all" in evaluation:
            eval = 0
        elif "Superficially relevant" in evaluation:
            eval = 1
    ...
    with open(f"/home/minhae/multihop-RAG2/ablation/gpqa/llama/{args.mode}_results_4.json", "w") as file:
